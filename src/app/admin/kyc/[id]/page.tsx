...
export default function AdminKycRoom() {
    ...
    useEffect(() => {
        ...
        startAndJoin()
        return () => {
            if (zpRef && typeof zpRef.destroy === 'function') zpRef.destroy()
        }
    }, [id])
    ...
    const handleAction = async (action: 'approve' | 'reject') => {
        ...
        setSubmitting(true)
        try {
            const res = await fetch(`/api/admin/reviews/partner/${id}/kyc`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
        ...
        <div className='flex h-screen bg-[#080808] text-white'>
            ...
            <div className='w-80 flex flex-col border-l border-white/10 shrink-0'>
                ...
                <header className='p-5 border-b border-white/10'>
                    ...
                    <p className='text-xs text-gray-500 mt-1'>Partner ID: {id.slice(-8).toUpperCase()}</p>
                </header>
                ...
                <div className='flex-1 p-5 overflow-y-auto space-y-5'>
                    <div className='p-4 bg-white/[0.03] rounded-xl border border-white/10'>
                        ...
                        <ul className='text-xs text-gray-400 space-y-2 list-disc list-inside'>
                            <li>Verify the partner's identity matches their CNIC profile.</li>
                            <li>Confirm their vehicle matches the uploaded RC documents if requested.</li>
                            <li>Ensure clear audio and professional behavior.</li>
                        </ul>
                    </div>
                </div>
                ...
                <div className='p-5 border-t border-white/10 bg-white/[0.02]'>
                    ...
                    <AnimatePresence>
                        {showRejectInput && (
                            <div className='mb-4 space-y-2'>
                                <label className='text-xs font-semibold text-gray-300'>Rejection Reason</label>
                                <textarea
                                ...
                                <div className='flex gap-2 pb-2'>
                                    <button onClick={() => setShowRejectInput(false)} className='flex-1 py-2 rounded-lg border border-white/10 text-xs hover:bg-white/5'>Cancel</button>
                                    <button onClick={() => handleAction('reject')} disabled={!rejectReason || submitting} className='flex-1 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white font-bold text-xs disabled:opacity-50'>Confirm Reject</button>
                                </div>
                            </div>
                        )}
                    </AnimatePresence>
                    ...
                    {!showRejectInput && (
                        <div className='flex flex-col gap-3'>
                            ...
                            <button
                                onClick={() => setShowRejectInput(true)} disabled={submitting}
